...
# ------------------------------------------------------------------------------
singularity exec \
    --bind $mntroot ${containerdir}/amrfinderplus.sif \
    amrfinder_update \
    -d ${assdir}/amrfinder_db
...
singularity exec \
    --bind $mntroot ${containerdir}/blast.sif \
    bash ${assdir}/mlst_db/mlst-make_blast_db.sh &> /dev/null
...
singularity exec \
    --bind $mntroot ${containerdir}/pythonScripts.sif \
    python3 INSTALL.py \
    ${assdir}/kma/kma_index

cd ${assdir}/resfinder_db
singularity exec --bind \
    $mntroot ${containerdir}/pythonScripts.sif python3 \
    INSTALL.py ${assdir}/kma/kma_index
...
# ===============================================================================
# Set up reference genomes for various organisms
# ===============================================================================
...
singularity exec \
    ...
    -i NC_002951.2 \
    -o ${assdir}/genomes/staphylococcus_aureus

# Index reference
cd ${assdir}/genomes/staphylococcus_aureus
singularity exec \
    --bind $mntroot ${containerdir}/bwakit.sif \
    bwa index \
    NC_002951.2.fasta
...
singularity exec \
    --bind $mntroot ${containerdir}/chewbbaca.sif \
    chewie PrepExternalSchema \
    -i ${assdir}/cgmlst/staphylococcus_aureus/alleles \
    -o ${assdir}/cgmlst/staphylococcus_aureus/alleles_rereffed \
	--cpu 1 \
    --ptf ${assdir}/prodigal_training_files/Staphylococcus_aureus.trn
...
singularity exec \
    ...
    python3 bin/download_ncbi.py \
    -i NC_000913.3 \
    -o ${assdir}/genomes/escherichia_coli
...
singularity exec \
    ...
    -sp 5 \
    -sc 1 \
    -o ${assdir}/wgmlst/escherichia_coli/alleles \
    --latest
...
singularity exec \
    --bind $mntroot ${containerdir}/chewbbaca.sif \
    chewie PrepExternalSchema \
    -i ${assdir}/cgmlst/escherichia_coli/alleles \
    -o ${assdir}/cgmlst/escherichia_coli/alleles_rereffed \
	--cpu 1 \
    --ptf ${assdir}/prodigal_training_files/Escherichia_coli.trn
...
singularity exec \
    ...
    python3 bin/download_ncbi.py \
    -i NC_016845.1 \
    -o ${assdir}/genomes/klebsiella_pneumoniae
...
# Download K. pneumoniae cgmlst cgmlst.org schema
...
# bwa check
ref=${assdir}/genomes/staphylococcus_aureus/NC_002951.2.fasta;
refamb=$ref.amb;
refann=$ref.ann;
refbwt=$ref.bwt;
refpac=$ref.pac;
refsa=$ref.sa
if [[ -f $ref && -f $refamb && -f $refann && -f $refbwt && -f $refpac && -f $refsa ]]; then
...
# BlastDB check
mlst=${assdir}/mlst_db/blast/mlst.fa;
mlstndb=$mlst.ndb;
mlstnhd=$mlst.nhd;
mlstnhi=$mlst.nhi;
mlstnhr=$mlst.nhr;
...
mlstntf=$mlst.ntf;
mlstnto=$mlst.nto
if [[ -f $mlst && -f $mlstndb && -f $mlstnhd && -f $mlstnhi && -f $mlstnhr && -f $mlstnin && -f $mlstnog && -f $mlstnos && -f $mlstnot && -f $mlstnsq && -f $mlstntf && -f $mlstnto ]]; then
